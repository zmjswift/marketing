    ...
    public function render() {
      ...
      if( ! empty( $this->parent ) && preg_match( '/'. preg_quote( '['. $this->field['id'] .']' ) .'/', $this->parent ) ) {
        ...
        echo '<div class="csf-cloneable-item csf-cloneable-hidden">';
          ...
          echo '<i class="csf-cloneable-remove csf-confirm fa fa-times" data-confirm="'. esc_html__( '确定要删除此项目吗？', 'csf' ) .'"></i>';
          echo '</div>';
        ...
        if( ! empty( $this->value ) ) {
          ...
          foreach ( $this->value as $value ) {
            ...
            echo '<div class="csf-cloneable-item">';
              ...
              echo ( $title_prefix ) ? '<span class="csf-cloneable-title-prefix">'. $title_prefix .'</span>' : '';
              echo ( $title_auto ) ? '<span class="csf-cloneable-value">模块ID：' . $first_value .'</span>' : '';
              echo '</span>';
              echo '</h4>';
        ...
        echo '<div class="csf-cloneable-alert csf-cloneable-max">'. esc_html__( '不能添加超过', 'csf' ) .' '. $args['max'] .'</div>';
        echo '<div class="csf-cloneable-alert csf-cloneable-min">'. esc_html__( '不能删除小于', 'csf' ) .' '. $args['min'] .'</div>';
