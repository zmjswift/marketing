  ...
  class CSF_Field_media extends CSF_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'preview'      => true,
        'library'      => array(),
        'button_title' => esc_html__( '上传', 'csf' ),
        'remove_title' => esc_html__( '删除', 'csf' ),
      ...
      //$preview_src = ( $args['preview_size'] !== 'thumbnail' ) ? $this->value['url'] : $this->value['thumbnail']; // 获取缩略图
      $preview_src = $this->value['url']; // 获取完整图片
      $hidden_url  = ( empty( $args['url'] ) ) ? ' hidden' : '';
      $hidden_auto = ( empty( $this->value['url'] ) ) ? ' hidden' : '';
      $placeholder = ( empty( $this->field['placeholder'] ) ) ? ' placeholder="'.  esc_html__( '未选择媒体文件', 'csf' ) .'"' : '';
