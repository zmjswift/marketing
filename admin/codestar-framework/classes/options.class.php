...
if( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public $args         = array(
      ...
      // menu extras
      'show_bar_menu'           => false,
      'show_sub_menu'           => false,//是否显示设置子菜单
      'show_network_menu'       => true,
      'show_in_customizer'      => false,

      'show_search'             => false,//搜索设置选项
      'show_reset_all'          => true,
    ...
    public function set_options() {
      ...
      if( wp_verify_nonce( csf_get_var( 'csf_options_nonce'. $this->unique ), 'csf_options_nonce' ) ) {
        ...
        if( ! empty( $transient['csf_import_data'] ) ) {
          ...
          $this->notice = esc_html__( '已全部恢复至默认设置', 'csf' );
          ...
          $this->notice = esc_html__( '仅还原此部分的默认设置', 'csf' );
        ...
        if( empty( $this->notice ) ) {
          $this->notice = esc_html__( '设置已保存', 'csf' );
        }
    ...
    public function add_admin_menu() {
      ...
      if( $menu_type === 'submenu' ) {
        ...
        $menu_page = call_user_func( 'add_submenu_page', $menu_parent, $menu_title, $menu_title, $menu_capability, $menu_slug, array( &$this, 'add_options_html' ), 2 );
    ...
    public function add_options_html() {
      ...
      echo '<div class="csf csf-options'. $theme . $class . $wrapper_class .'" data-slug="'. $this->args['menu_slug'] .'" data-unique="'. $this->unique .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<h1 style="text-transform: uppercase">网站优化设置</h1>';
          echo '</div>';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( '设置已更改，请及时保存设置！', 'csf' ) .'</div>' : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( '显示所有选项', 'csf' ) .'"><i class="fa fa-outdent"></i></div>' : '';

            echo ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="'. esc_html__( '搜索选项', 'csf' ) .'" autocomplete="off" /></div>' : '';

            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. $this->unique .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. $ajax_class .'" value="'. esc_html__( '保存设置', 'csf' ) .'" data-save="'. esc_html__( '正在保存...', 'csf' ) .'">';
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( '重置当前区域设置', 'csf' ) .'" data-confirm="'. esc_html__( '确定要重置当前区域设置吗？', 'csf' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. esc_html__( '全部重置', 'csf' ) .'" data-confirm="'. esc_html__( '确定要重置所有选项吗？', 'csf' ) .'">' : '';
            ...
            if(class_exists('Memcached') && wp_using_ext_object_cache()) {
        		...
        		$current_admin_url = home_url(add_query_arg(array()));
            if( isset($_GET['action']) ){
          		//$action	= $_GET['action'] ?: '';
          		if($_GET['action'] == 'refresh'){
          			check_admin_referer('refresh-memcached');
          			wp_cache_flush();
  	
          			wp_redirect(add_query_arg( array( 'deleted' => 'true' ), _get_referer() ));
          		}
            }?>
        		<a type="submit" href="<?php echo esc_url(wp_nonce_url($current_admin_url.'&action=refresh', 'refresh-memcached'));?>" class="button button-primary" style="background:#FF9800;border-color:#FF9800;"><i style="font-size:14px;vertical-align:middle;line-height:16px;" class="dashicons dashicons-image-rotate"></i> 刷新缓存</a>
            <?php
            }
        ...
        echo '<div class="cs-block-left">感谢使用'.wp_get_theme().'主题，当前版本：V'. wp_get_theme()->get( 'Version' ).'，您的网站运行在： WordPress '. get_bloginfo('version') .' / PHP '. PHP_VERSION .'</div>';
        echo '<div class="cs-block-right">&copy; '.date('Y').' Theme By <a href="http://www.dqtheme.com" target="_blank" style="color: #555;text-decoration: none;">www.dqtheme.com</a> </div>';
        echo '<div class="clear"></div>';
        echo '</footer><style>.cs-block-right{text-align:right}.cs-block-left{float:left}</style>'; // end .cs-footer

          //echo '<div class="csf-buttons">';
          //echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. $ajax_class .'" value="'. esc_html__( '保存设置', 'csf' ) .'" data-save="'. esc_html__( '正在保存...', 'csf' ) .'">';
          //echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( '重置当前区域设置', 'csf' ) .'" data-confirm="'. esc_html__( '确定要重置当前区域设置吗？', 'csf' ) .'">' : '';
          //echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. esc_html__( '全部重置', 'csf' ) .'" data-confirm="'. esc_html__( '确定要重置所有选项吗？', 'csf' ) .'">' : '';
          //echo '</div>';
